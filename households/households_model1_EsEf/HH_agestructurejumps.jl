...
function rates(out,u,p::CoVParameters_AS,t)
    ...
    for k = 1:length(out)
        ...
        if eventtype ==2
            out[k] = (1-(δ*rel_detection_rate[a]))*σ*u[i,a,2] # E->A    #HH becomes Eᶠ->A (without modifs here)
        end
        if eventtype ==3
            out[k] = δ*rel_detection_rate[a]*σ*u[i,a,2] # E->D          #HH becomes Eᶠ->D (without modifs here)
        end
        if eventtype ==4
            out[k] = τ*u[i,a,4] # D->H
        end
...
function max_change(out,u,p::CoVParameters_AS)
    for i = 1:n_wa,a = 1:n_a
        ...
        ind_trans2 = linear_as_events[i,a,9]            #HH
        ind_EA2 = linear_as_events[i,a,10]              #HH
        ind_ED2 = linear_as_events[i,a,11]              #HH
