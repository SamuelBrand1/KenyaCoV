using Plots,MAT, StatsPlots, Statistics,JLD2,CSV,DataFrames,Printf,ColorBrewer,Colors#, Images, ImageView,ImageDraw
    ...
    StatsPlots.default(grid=false,legendfontsize=6, legendfontcolor=:gray30,titlefontsize=8, titlefontcolor=:gray30,xtickfontsize=6, ytickfontsize=6,tickfontcolor=:gray30,linewidth=false)
...
function cumIandDeaths_kenya(data_files,plot_folder,S0)
    ...
    barplotALL=bar([1.5:2:20*2;],S02,color=[:green,:chartreuse4],xticks=([1:2:20*2;],riskregionnames),fillalpha=.5,linewidth=false,bar_width=2,linecolor=false,
                    size=(width1*1.1,length1),rotation=40,label=false,title="Estimated cases and deaths in Kenya without intervention")
    ...
    barplotALL2=bar([1.5:2:47*2;],S02_counties,color=[:green,:chartreuse4],xticks=([1:2:47*2;],counties_names),fillalpha=.5,linewidth=false,bar_width=2,linecolor=false,
                    widen=true,xrotation=70,label=false,title="Estimated cases and deaths in Kenya without intervention",size=(width1*1.5,length1*.8))
...
#bar([0 for i=1:47*2],xticks=([1:2:47*2;],counties_names),rotation=90,yaxis=false,widen=true,size=(400,200))
function cumIandDeaths_ages_kenya(data_files,plot_folder,S0)
    if !isdir(plot_folder)   mkdir(plot_folder)   end
    #wa=12;wa_name="Kilifi"

    @load data_files[1]  sims_vector # No detection R01 and δ1 (only calculating deaths)
    cumDeaths_1=[[] for i=1:16];
    for sim=1:size(sims_vector,1),a=1:16    push!(cumDeaths_1[a],sims_vector[sim][11][:,a][1]); end

    @load data_files[2]  sims_vector # No detection R01 and δ2 (calculating cumI and deaths)
    ...
    for sim=1:size(sims_vector,1),a=1:16    push!(cumDeaths_2[a],sims_vector[sim][11][:,a][1]); end

    @load data_files[3]  sims_vector # No detection R02 and δ1 (only calculating deaths)
    cumDeaths_3=[[] for i=1:16];
    for sim=1:size(sims_vector,1),a=1:16    push!(cumDeaths_3[a],sims_vector[sim][11][:,a][1]); end

    @load data_files[4]  sims_vector## No detection R01 and δ2 (calculating cumI and deaths)
    ...
    for sim=1:size(sims_vector,1),a=1:16    push!(cumDeaths_4[a],sims_vector[sim][11][:,a][1]); end
    ...
    b=bar([1.5:2:16*2;],S0perAge,color=[:green,:chartreuse4],xticks=([1.5:2:16*2;],ages),fillalpha=.5,linewidth=false,bar_width=2,linecolor=false,
                    size=(width1*1.1,length1),rotation=40,label=false,title="Estimated cases and deaths per age in Kenya without intervention")

    cumIfinal=[];    for a=1:16  push!(cumIfinal,median(cumIsA_2[a]));push!(cumIfinal,median(cumIsA_4[a])); end # collecting only for δ2 (10%)
    bar!(b,cumIfinal,color=[c_R01_I,c_R02_I],rotation=30,contour_labels=true,size=(width1*1.2,length1*.8),linecolor=false,label=false);#display(b)
    ...
    bar!(b,cumDeathsfinal,color=:red,xrotation=70,size=(width1*1.2,length1*.8),linecolor=false,label=false);#display(b)
    ...
    bar!(bdeaths,[cumDeathsfinal_4files[i] for i=1:2:16*4],color=[c_R01_I,c_R02_I],ls=:dash,xaxis=:false,size=(width1*1.15,length1*.4),bar_widths=.6,label=false);#display(bdeaths)
...
function make_plots_1R0_twoδs_Kenya(folders,plot_folder,S0,δs,reference_files)
    ...
    boxplot!(b7,xticks=([0:1:size(xticks_labels,1);], xticks_labels),size=(width1,length1),xrotation=Rotation)
        b2_v2=bar(cumI_diffs[n_scenarios+2:end],color=colors2_subset,ls=:solid,title="Gain in number of cases in Kenya - R0="*string(sessionParams.R₀),fillalpha=.8,
        ...
        bar!(b2_v2,cumI_diffs[2:n_scenarios],color=colors2_subset,ls=:dash,lw=0,bar_widths=.5,legend=false,size=(width1,length1),xrotation=Rotation)
        b3_v2=bar(cumIs[1:n_scenarios],color=[colors2_subset[end],colors2_subset[1],colors2_subset[2]],ls=:dash,title="Estimated cases in Kenya - R0="*string(sessionParams.R₀),fillalpha=.8,ylims=(1.5e7,2.3e7),
                    legend=false,xticks=([0:1:size(cumI_diffs[n_scenarios+1:end],1);], [xticks_labels[1];xticks_labels[2:end]]),size=(width1*.8,length1*.8),xrotation=Rotation)
        ...
        b10=bar(cumDeaths[n_scenarios+1:end],color=[colors2_subset[end],colors2_subset[1],colors2_subset[2]],ls=:solid,title="Estimated death counts in Kenya - R0="*string(sessionParams.R₀),fillalpha=.8,ylims=(1e4,5.1e4),
                legend=false,xticks=([0:1:size(cumDeaths[n_scenarios+1:end],1);], [xticks_labels[1];xticks_labels[2:end]]),size=(width1*.8,length1*.8),xrotation=Rotation)
...
function CT_comparedtoNoIntervention_area(no_int_data_file,data_file,plot_folder,withLabel)
    ...
    @load no_int_data_file  sessionParams;    @load no_int_data_file  sims_vector
    ...
    @load data_file  sessionParams;    @load data_file  sims_vector
    ...
    plot!(p,IKenya_nointervention_median,ribbon=(IKenya_nointervention_min,IKenya_nointervention_max),color=:red,label="No interv")
    plot!(p,IKenya_median,ribbon=(IKenya_min,IKenya_max),color=:blue,label="Intervention",legend=withLabel)#;display(p)
    ...
    for folder in folders
        ...
        for data_file in data_files
            ...
            for wa=1:size(riskregionnames,2)-1
                p_curves=[plot(size=(width1,length1)) for i=1:6]
                for sim=1:size(sims_vector,1),i=1:6
                    d=[sims_vector[sim][i][t][wa][1] for t=1:366]
                    plot!(p_curves[i],d,color=colors2[i])
                end
                for i=1:6   savefig(p_curves[i],plot_folder2*string(i)*"_"*curve_names[i]*"_wa"*string(wa)*"_"*replace(riskregionnames[wa],"/"=>"-")*".png")   end
            end
        end
    end
end
