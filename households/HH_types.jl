...
@with_kw mutable struct CoVParameters_AS
    ...
    dt::Float64 = 1. #Useful for the non-negative method
    #Î::Matrix{Float64} = zeros(n_wa,n_a) #For inplace calculations    #HH
    Îᶠ::Matrix{Float64} = zeros(n_wa,n_a) #For inplace calculations    #HH
    Îˢ::Matrix{Float64} = zeros(n_wa,n_a) #For inplace calculations    #HH
