...
"""
@with_kw mutable struct CoVParameters_Vaccination
    ...
    #Vaccination
    v_mode::Int64 = 1   # 1->use the vaccination rate v / 2->use the age-specified vaccination rate vₐ / 3->use the vaccination protocol vᵗ[t,i,a] (number to vaccinate at day t)
    v::Float64 = 1/300.   #vaccination rate
    vₐ::Matrix{Float64} = zeros(n,n_a)   #age-specified vaccination rate  / or used for inplace calculations as the number of vaccinateds at a specific time
    sim_dur::Int64=370
    vᵗ::Array{Int64,3} = zeros(Int64,sim_dur,n,n_a)  #Vaccination protocol: vᵗ[t,i,a] is the number of people to vaccinate at day t
    s::Float64 = .0     #loss of immunity rate
end
