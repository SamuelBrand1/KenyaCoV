...
9-> S to R  # Vaccination
10->R to S  # Loss of immunity
...
# diseased_indices = 0;
"""
    function calculate_infection_rates!(u,p::CoVParameters_Vaccination,t)
...
"""
    function rates(out,u,p::CoVParameters_Vaccination,t)
...
"""
function rates(out,u,p::CoVParameters_Vaccination,t)
    @unpack λ,γ,σ₁,σ₂,δ,τ,μ₁,χ,rel_detection_rate,clear_quarantine,hₐ,v,s = p
    ...
    for k = 1:length(out)
        ...
        if eventtype ==9
            out[k] = v*u[i,a,1] # S->R  Vaccination
        end
        if eventtype ==10
            out[k] = s*u[i,a,8] # R->S  Loss of immunity
        end
    end
end
...
function change_matrix(dc)
    ...
    for k = 1:d2 #loop over
        ...
        if eventtype ==9 # S->R  Vaccination
            ind_S = linear_as[i,a,1]
            ind_R = linear_as[i,a,8]
            dc[ind_S,k] = -1
            dc[ind_R,k] = 1
        end
        if eventtype ==10 # R->S  Loss of immunity
            ind_R = linear_as[i,a,8]
            ind_S = linear_as[i,a,1]
            dc[ind_R,k] = -1
            dc[ind_S,k] = 1
...
rates calculated by rates(out,u,p::CoVParameters_Vaccination,t).
"""
function PP_drivers(dN::Vector{Int64},rates,p)
    for i = 1:length(dN)
        if rates[i] >= 0.
...
"""
    function max_change(out,u,p::CoVParameters_Vaccination)
...
function max_change(out,u,p::CoVParameters_Vaccination)
    ...
    for i = 1:n,a = 1:n_a
        ...
        ind_SR = linear_as_events[i,a,9]    #Vacc
        ind_RS = linear_as_events[i,a,10]   #Loss of immunity

        #out[ind_trans] = min(out[ind_trans],u[i,a,1])
        if out[ind_trans] + out[ind_SR] > u[i,a,1]     #Vaccination events S->R
            out[ind_SR]=rand(binomial(u[i,a,1],LinearAlgebra.normalize!([v,1-v],1)))
            out[ind_trans]=u[i,a,1]-out[ind_SR]
        end
        ...
        out[ind_RS] = min(out[ind_RS],u[i,a,8])
...
"""
    nonneg_tauleap(du,u,p::CoVParameters_Vaccination,t)

This performs one in-place simulation of a time step
"""
function nonneg_tauleap(du,u,p::CoVParameters_Vaccination,t)
...
"""
    function ode_model(du,u,p::CoVParameters_Vaccination,t)

This is the vector field of the related KenyaCoV_vaccination ODE model
"""
...
function get_vᵗ(t,i,a,p)
    if t>=1 && t==floor(t)
        return p.vᵗ[Int(t),i,a]
    else return 0
    end
end

function ode_model_vacc(du,u,p::CoVParameters_Vaccination,t)
    @unpack λ,γ,σ₁,σ₂,δ,τ,μ₁,χ,rel_detection_rate,hₐ,v,vₐ,vᵗ = p
    calculate_infection_rates!(u,p,t)
    n,n_a,n_s = size(u)
    #if t>=1 && t==floor(t)        println(t)    end#,"\t\t",sum(vᵗ[Int(t),:,:]))    end
    for i = 1:n,a = 1:n_a
        #du[i,a,1] = (-1)*χ[a]*λ[i,a]*u[i,a,1] - v*u[i,a,1] #+ s*u[i,a,8]   ## v is a vaccination rate (Float64)
        #du[i,a,1] = (-1)*χ[a]*λ[i,a]*u[i,a,1] - vₐ[i,a]*u[i,a,1]          ## vₐ[i,a] is the rate of vaccination
            du[i,a,1] = (-1)*χ[a]*λ[i,a]*u[i,a,1] - min(vₐ[i,a],u[i,a,1])          ## vᵗ[t,i,a] is the number of people to vaccinate at day t
        ...
        du[i,a,6] = σ₂*δ*hₐ[a]*rel_detection_rate[a]*u[i,a,3] - τ*u[i,a,6]
        du[i,a,7] = τ*u[i,a,6]
        #du[i,a,8] = γ*u[i,a,5] + γ*u[i,a,4] + vₐ[i,a]*u[i,a,1]             ## vₐ[i,a] is the rate of vaccination
            du[i,a,8] = γ*u[i,a,5] + γ*u[i,a,4] + min(vₐ[i,a],u[i,a,1])          ## vᵗ[t,i,a] is the number of people to vaccinate at day t
