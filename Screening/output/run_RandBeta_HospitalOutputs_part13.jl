...
session=06  ;    scenarios=[i  for i=1:15]
    ...
    @time for i=[1,2,3,4,6,7,8,9]#size(scenarios,1)
        ...
        #for r=1:KenyaCoV_screening.n_wa P.S_strategy[r,:]=MS_strategies[i];  end #MS same in all kenya
        P.S_strategy[30,:]=S_strategies[i];P.S_strategy[28,:]=S_strategies[i];#MS in Nairobi and Monbassa only
