...
######## Intervention 2 : Symptomatic screening
session=02  ;    scenarios=[i  for i=1:15]
    folder="./Screening/output/2_SympS/session"*string(session)*"_"*string(n_traj)*"sims/";if !isdir(folder)   mkdir(folder)   end
    @time for i=[1,2,3,4,6,7,8,9]#size(scenarios,1)
        println("Intervention=2_SympS\tsession=",session,"\tsc=",scenarios[i],"\tn_traj=",n_traj)
        u0,P=deepcopy(u0_0),deepcopy(P_0)
        #for r=1:KenyaCoV_screening.n_wa P.S_strategy[r,:]=MS_strategies[i];  end # same in all kenya
        P.S_strategy[30,:]=S_strategies[i];P.S_strategy[28,:]=S_strategies[i];# in Nairobi and Monbassa only
        cb=CallbackSet(callback_selection_and_mvt_probabilities_symptomatics,callback_SympS)
        ...
        @save folder*"SympS_sc"*string(scenarios[i])*"_"*string(n_traj)*"sims.jld2" results
    end
println();println();
