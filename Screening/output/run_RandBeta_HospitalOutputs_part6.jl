...
session=05  ;    scenarios=[i  for i=1:15]
    ...
    @time for i=[1,2,3,4,6,7,8,9]#size(scenarios,1)
        ...
        #P.S_strategy[4,:]=MS_strategies[i];P.S_strategy[12,:]=MS_strategies[i];# in Nairobi and Monbassa only
        cb=CallbackSet(callback_selection_and_mvt_probabilities_symptomatics,callback_SympS,callback_CT)
        ...
        @save folder*"SympSCT_sc"*string(scenarios[i])*"_"*string(n_traj)*"sims.jld2" results
    end
println();println();
