#=@everywhere =#push!(LOAD_PATH, "./src")
    #=@everywhere =#push!(LOAD_PATH, "./Screening_CTC")
...
#@everywhere begin
        ...
        #include("./Screening_CTC/callbacks_screening.jl")
        include("../../callbacks_screening.jl")
###### PARAMS TO MODIFY
    code_folder="/home/gemvi/rabia_aziza_midasnetwork_us/julia_projects/2020-07-22_KenyaCoV/Screening_CTC/simFiles_5/fixed_K_1000tests/"
        code_folder="./Screening_CTC/simFiles_5/fixed_K_1000tests/"
    output_folder="./Screening_CTC/output_fixed_K_1000tests/"
    n_traj=1
    ϵ=1.
...
######
    ...
    if !isdir(output_folder)   mkdir(output_folder)   end
        ...
        if !isdir(output_folder*"I6_CTH_CTC/")   mkdir(output_folder*"I6_CTH_CTC/")   end
        if !isdir(output_folder*"I7_SympSCT_CTC/")   mkdir(output_folder*"I7_SympSCT_CTC/")   end
        if !isdir(output_folder*"I8_MSCT_CTC/")   mkdir(output_folder*"I8_MSCT_CTC/")   end


    function run_intervention_session(session,scenarios,intervention_label,nb_months,cb,n_traj;CT_n_decaying=0,test_and_trace_contacts=false)
        ...
        @time for i ∈ scenarios
            ...
            P.test_and_trace_contacts=test_and_trace_contacts
