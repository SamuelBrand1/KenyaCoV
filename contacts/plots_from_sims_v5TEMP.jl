...
function cumIgain_many_sessions(sessions,τₚ_list,results_folder,data_files,n_traj,wa_coords,wa,wa_name)
    bar_vector=[]
    colors2=[]
    ticks2=[]
    for j=2:size(τₚ_list,1),i=1:size(sessions,1)
        ...
        push!(ticks2,string(τₚ_list[j]*100)*"% Isolated "*string(Κ_max_capacity_Kilifi[i]))
    end
#
    b=bar(bar_vector,color=colors2,legend=false,title="Gain in final cumulative Infecteds in Kilifi (all ages)",
            xticks = ([1:1:size(bar_vector,1);], ticks2),rotation = 60)
    display(b)
