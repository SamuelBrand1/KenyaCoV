...
function calculate_infection_rates!(u,p::CoVParameters_AS,t)
    Iᴬ = @view u[:,:,3]
    Iᴰ = u[:,:,4] .+ u[:,:,10]                                                 #**** also summing for IQ
...
function max_change(out,u,p::CoVParameters_AS)
    for wa=1:n_wa,a=1:n_a,s=1:n_s
        if u[wa,a,s]<0                          u[wa,a,s]=0  end
        if out[linear_as_events[wa,a,s]]<0      out[linear_as_events[wa,a,s]]=0  end
    end
    for i = 1:n_wa,a = 1:n_a
        ...
        if u[i,a,2]<0   println("ERROOOOR n >= zero(n)")    end
    ...
    for wa=1:n_wa,a=1:n_a,s=1:n_s
        if out[linear_as_events[wa,a,s]]<0  #println("NEGATIVE ERROR: event dN[",wa,",",a,",",s,"]");
...
function IQ_make_contacts(u,p::CoVParameters_AS,t::Float64)                     #****
    ...
    for wa=1:n_wa,a=1:n_a,s=1:n_s   ## Recalculate uₚ
        if s∉[5,7,8,9]      uₚ[wa,a,s]=u[wa,a,s]/(sum(u[wa,a,1:4])+u[wa,a,6]+u[wa,a,10]) ## we only include the states 1(S) 2(E) 3(Iᴬ) 4(Iᴰ) 6(R) and 10(IQ). We do not include 5(Q) nor the cumulatives.
        else                uₚ[wa,a,s]=0;   end  # we have no chance of meeting an Q, and we remove the cumulative states
    end
...
function IQ_make_contacts_Nairobi(u,p::CoVParameters_AS,t::Float64)                     #****
    @unpack l_IQ,κ,Mₚ,dt,uₚ = p
    #### Recalculate uₚ=zeros(n_wa,n_a,n_s)   : Matrix of probabilities: when contacting someone with a specific wa and a, what is the chance of him being S, E, IQ, Iᴰ,... WE DO NOT MEET Q!
    for wa=4,a=1:n_a,s=1:n_s   ## Recalculate uₚ
        if s∉[5,7,8,9]      uₚ[wa,a,s]=u[wa,a,s]/(sum(u[wa,a,1:4])+u[wa,a,6]+u[wa,a,10]) ## we only include the states 1(S) 2(E) 3(Iᴬ) 4(Iᴰ) 6(R) and 10(IQ). We do not include 5(Q) nor the cumulatives.
        else                uₚ[wa,a,s]=0;   end  # we have no chance of meeting an Q, and we remove the cumulative states
    end
...
function update_l_IQ(dN::Vector{Int64},u,p::CoVParameters_AS,t)                 #****
    ...
    for wa=1:n_wa,a=1:n_a
        ...
        for i=1:n_Iq_to_R
            ...
            if size(selected_IQ_list,1)==0 println("none found!")
            else
    ...
    for i=1:size(l_IQ,1)
        if l_IQ[i].detection_dur-p.dt>=0     l_IQ[i].detection_dur-=p.dt;
        else                l_IQ[i].detection_dur=0;           end
    end

    ## Add all new IQ to the list l_IQ l_IQ
    for wa=1:n_wa,a=1:n_a
        if dN[linear_as_events[wa,a,9]]!=0
            for exp=1:dN[linear_as_events[wa,a,9]]      push!(l_IQ,IQ_Person(wa=wa,a=a,detection_dur=randexp()/τ) );  end     ##need to rescale by dt??
        end
    end
end
    ...
    for i=1:size(l_IQ,1) ## for all current IQ
        ...
        while j<=size(l_IQ[i].contacts,1) ## for all contacts
            ...
            elseif e.contact_t!=t         ## if the contact hasnt been made during the current timestep
                #if dN[linear_as_events[e.wa,e.a,1]]/u[e.wa,e.a,1]<0 println("ERROOOOR 3")   end
                if u[e.wa,e.a,1]!=0 && e.s==1 && rand(Binomial(1,dN[linear_as_events[e.wa,e.a,1]]/u[e.wa,e.a,1]))==1    e.s=2   end ## If contact is S(1) and event S->E(1) occured
                if u[e.wa,e.a,2]!=0 && e.s==2 ## if contact is E(2) then he can have E->Iᴬ(event 2), E->Iᴰ(3) or E->IQ(9)
                    ...
                    if      new_ps==2    e.s=3
                    elseif  new_ps==3    e.s=4
                    elseif  new_ps==4    e.s=10
                    end
                end
                #if dN[linear_as_events[e.wa,e.a,7]]/u[e.wa,e.a,3]<0 && dN[linear_as_events[e.wa,e.a,7]]/u[e.wa,e.a,3]>1 println("ERROOOOOR")    end
                #if dN[linear_as_events[e.wa,e.a,7]]/u[e.wa,e.a,3]>=0 && dN[linear_as_events[e.wa,e.a,7]]/u[e.wa,e.a,3]<=1
                if u[e.wa,e.a,3]!=0 && e.s==3 && rand(Binomial(1,dN[linear_as_events[e.wa,e.a,7]]/u[e.wa,e.a,3]))==1    e.s=6   end ## If contact is Iᴬ(3) and event Iᴬ->R(7) occured
                #else    println("ERROR 4: ",u[e.wa,e.a,3]); end
                if u[e.wa,e.a,4]!=0 && e.s==4 && rand(Binomial(1,dN[linear_as_events[e.wa,e.a,6]]/u[e.wa,e.a,4]))==1    e.s=6   end ## If contact is Iᴰ(4) and event Iᴰ->R(6) occured
                if u[e.wa,e.a,5]!=0 && e.s==5 && rand(Binomial(1,dN[linear_as_events[e.wa,e.a,5]]/u[e.wa,e.a,5]))==1    e.s=6   end ## If contact is Q(5) and event Q->R(5) occured
                ...
                if u[e.wa,e.a,10]!=0 && e.s==10 ## if contact is IQ then he can have IQ->Q(event 4) or IQ->R(event 10)
                    ...
                    if p_Q>=0 && p_R>=0 && p_IQ>=0
                        #println([p_IQ,p_Q,p_R])
                        new_ps=random_choice([p_IQ,p_Q,p_R])
                        if      new_ps==2    e.s=5
                        elseif  new_ps==3    e.s=6
                        end
                    end
                end
            end
...
function intervention_trace_contacts(traced_index,u,p::CoVParameters_AS,t)      #****
    @unpack l_IQ,Δₜ,κₜ = p
    contacteds=[c  for c in l_IQ[traced_index].contacts if t-c.contact_t<=Δₜ]
    if contacteds!=[]
        shuffle!(contacteds)
...
function check_negativity(u,dN,t)
    for wa=1:n_wa,a=1:n_a,s=1:n_s
        if u[wa,a,s]<0      #println("At     ",t," , u[",wa,",",a,",",s,"]=",u[wa,a,s]);
            u[wa,a,s]=0  end
        if dN[linear_as_events[wa,a,s]]<0   dN[linear_as_events[wa,a,s]]=0  end
    end
end
