...
function run_session(session,Κ_max_capacity_KENYA,Κ_max_capacity_Nairobi,Κ_max_capacity_Kilifi,κ_per_event4,τₚ_list,n_traj,ϵ,σ,γ,δ,β,Δₜ,stop_Q,IDs_contacted_first)
    ...
    P.τ=1/3.;        P.κ=10;        P.κₘ=Δₜ;     P.Δₜ=Δₜ;     P.stop_Q=stop_Q;      P.IDs_contacted_first=IDs_contacted_first;
...
###########
sessions=[60,61,62,63]

Κ_max_capacity_KENYA=[0,0,0,0,0,0]; Κ_max_capacity_Nairobi=[0,0,0,0,0,0]
Κ_max_capacity_Kilifi=[500,1e3,5e3,1e4]
κ_per_event4=[100,100,100,100]
τₚ_list=[.0,.25,.5,.75,.9]
Δₜ=14
n_traj=10
stop_Q=true    #****5 do we stop detecting after we stopped tracing
IDs_contacted_first=false

for i=1:size(sessions,1)
    run_session(sessions[i],Κ_max_capacity_KENYA[i],Κ_max_capacity_Nairobi[i],Κ_max_capacity_Kilifi[i],κ_per_event4[i],τₚ_list,n_traj,ϵ,σ,γ,δ,β,Δₜ,stop_Q,IDs_contacted_first)
end

sessions=[64,65,66,67]
