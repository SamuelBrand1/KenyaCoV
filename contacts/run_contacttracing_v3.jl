...
P.Δₜ=3
P.κ_per_event4=30
P.Κ_max_capacity[KenyaCoV_contacts.ind_nairobi_as]=1e2  #!!!! Max capacity in Nairobi = 1000
P.Κ_max_capacity[KenyaCoV_contacts.ind_mombasa_as]=1e2  #!!!! Max capacity in Nairobi = 1000
...
function run_saveMAT(P::KenyaCoV_contacts.CoVParameters_AS,prob,n_traj,τₚ_list,session)
    folder="./contacts/results_session"*string(session)*"/"
    mkdir(folder)
    for τₚ in τₚ_list
        P.τₚ =τₚ
        println("Running ",n_traj," sims for τₚ=",τₚ, " and Κ_max_capacity=",P.Κ_max_capacity)
...
results_sessions = run_saveMAT(P,prob,10,τₚ_list,11)
