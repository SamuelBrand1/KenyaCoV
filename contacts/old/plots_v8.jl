...
function make_plots_twoδs(folders,plot_folder,S0,δs)
    ...
    for i=1:size(data_files,1)
        ...
        for sim=1:size(sims_vector,1) #for each sim
            ...
            I=[sims_vector[sim][5][t][wa][1] for t=1:size(sims_vector[sim][4],1)]
            tpeak=findall(x->x==maximum(I),I)[1]
            push!(peak,tpeak)
        ...
        push!(cumDeaths,median(cumDeath))
    ...
    b8=bar(cumCs[2:7],color=colors2,title="Total number of contacted in "*wa_name*" - R0="*string(sessionParams.R₀),#=lc=false,=#fillalpha=.8,#linewidth=false,
            legend=false,xticks=([0:1:size(cumI_diffs[2:7],1);], [xticks_labels[1];xticks_labels[3:end]]),size=(width1,length1),xrotation=Rotation)
        bar!(b8,cumCs[9:end],color=colors2,ls=:dash,lw=0,bar_widths=.5,
                legend=false,size=(width1,length1),xrotation=Rotation);#display(b8)
    b9=bar(cumICs[2:7],color=colors2,title="Contacted infecteds (E or Ix) in "*wa_name*" - R0="*string(sessionParams.R₀),#=lc=false,=#fillalpha=.8,#linewidth=false,
            legend=false,xticks=([0:1:size(cumI_diffs[2:7],1);], [xticks_labels[1];xticks_labels[3:end]]),size=(width1,length1),xrotation=Rotation)
    ...
    b10=bar(cumDeaths[9:end],color=colors2,ls=:dash,title="Total deaths in "*wa_name*" - R0="*string(sessionParams.R₀),#=lc=false,=#fillalpha=.8,#linewidth=false,
            legend=false,xticks=([0:1:size(cumDeaths[2:7],1);], [xticks_labels[1];xticks_labels[3:end]]),size=(width1,length1),xrotation=Rotation)
        bar!(b10,cumDeaths[2:7],color=colors2,ls=:solid,lw=0,bar_widths=.5,
                legend=false,size=(width1,length1),xrotation=Rotation);
...
folders=["./contacts/results_session20_10sims/","./contacts/results_session21_10sims/"];plot_folder="./contacts/results_session20_10sims/plots_sessions20-21/"
