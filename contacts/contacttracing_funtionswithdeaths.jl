...
function output_daily_and_final_incidence(sol,i)
    ...
    Q=[[sum(sol(t)[wa,:,5],dims = 1)[:,1,:] + sum(sol(t)[wa,:,11],dims = 1)[:,1,:] #=+ sum(sol(t)[wa,:,15],dims = 1)[:,1,:]=#  for wa=1:20]  for t in times]      # Q = Q + QS ###+ QA
    ...
    return [cumIs,cumC,cumIC,cumDeaths,I,IQ,Q,S,R,sol[end][:,:,7:9],sol[end][:,:,14]],false # save z (time series with only incidence with no age structure), and save the final distribution (end) age/space but no time
end
