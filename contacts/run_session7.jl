...
for j=1:10
    ...
    #display(plt);
    savefig(plt,"./contacts/results_plots_sessions/session7/detection_tests_"*string(P.τₚ)*"_v"*string(j)*".png")

    plot!(plt_Nairobi,sol.t,I_area[4,:],lab = "Nairobi"*string(j));
end
#display(plt_Nairobi);
savefig(plt_Nairobi,"./contacts/results_plots_sessions/session7/detection_tests_"*string(P.τₚ)*"_NAIROBI.png")
