...
plt = groupedbar(hcat(Kenya_PP,v,Case_Dist),lab = ["Pop. uniform AR" "Contact uniform AR" "Data"],
        xticks=(1:17,age_cats),
        size = (800,400),dpi=500,
...
function (cases::CaseDistribution)(θ) #Susceptibility form of likelihood
    @unpack χ,d,ϵ,R = θ               # extract the parameters θ is a generic label for any set of params, the difference is in the prediction function
    @unpack C, n, prediction,logprior = cases   # extract the data and prediction function
    T = eltype(χ)
    p = prediction(χ,d,ϵ,R) #predicted number of cases, asymptomatic and symptomatic
    ...
    try
        logL = 0.
        for i = 1:17
            logL += C[i,1]*log(1-pred_symp[i]) +  C[i,2]*log(pred_symp[i]) + obs_tot_cases_age[i]*log(pred_agedist[i]) + logprior(θ)  # C[,1] has number asymptomatic and C[,2] has number symptomatic. The third element of the log lik is a multinomial distribution of cases by age.
        end
        return logL
    catch errtype
        return T(-Inf)
...
function logprior_zhang(θ)
    χ,d,ϵ,R = θ.χ,θ.d,θ.ϵ,θ.R
    ll = 0.
    for a = 1:3
...
function pred_case_distribution_HH(χ::Vector,d::Vector,ϵ,R,C_work,C_other,H,SAR)
    d1, = size(C_work) # number of age groups
    K = zeros(eltype(d),d1,d1)
    for a = 1:d1,b=1:d1
        K[a,b] = χ[a]*(R*C_work[a,b] +C_other[a,b]  + SAR*H[a,b])*((1-d[b])*ϵ + d[b])
    end
    v = (K^10)*ones(d1)
    symp = d.*v  # predicted symptomatics cases
    asymp = v - symp # predicted asymptomatics cases
...
function HMC_for_detection_rate(n_draws,SAR)
    ...
    results = mcmc_with_warmup(Random.GLOBAL_RNG, ∇P, n_draws,
                                        initialization=(q = vcat(zeros(17),log.(0.1*ones(17)),log(1.),log(1.)),))#,reporter = NoProgressReport()) # reporter = NoProgressReport()
...
res = results_trans[1]
pred_distribs = [sum(pred_case_distribution_HH(res.χ,res.d,res.ϵ,res.R,M_Kenya_work ,M_Kenya_other,hh_mat_vector[30]',0.2),dims = 2)/sum(pred_case_distribution_HH(res.χ,res.d,res.ϵ,res.R,M_Kenya_work ,M_Kenya_other,hh_mat_vector[30]',0.2)) for res in results_trans]

data_distribution = sum(Kenya_Case_Dis_MAT,dims = 2)/sum(Kenya_Case_Dis_MAT)
groupedbar(hcat(data_distribution,mean(pred_distribs)),lab=["Data" "Fit"])
bar(mean([res.χ for res in results_trans]),xticks = (1:17,age_cats))
bar(mean([res.d for res in results_trans]))
mean([res.ϵ for res in results_trans])
mean([res.R for res in results_trans])

function reproductive_ratio(χ::Vector,d::Vector,ϵ,R,C_out,H,SAR)
    d1, = size(C_out) # number of age groups
    K = zeros(eltype(d),d1,d1)
    for a = 1:d1,b=1:d1
        K[a,b] = χ[a]*(R*C_out[a,b] + SAR*H[a,b])*((1-d[b])*ϵ + d[b])
    end
    evals, = eigen(K)
    return Real(evals[end])
end
mean([reproductive_ratio(res.χ,res.d,res.ϵ,res.R,0.5*M_Kenya_work,hh_mat_vector[30]',0.2) for res in results_trans])
M = hh_mat_vector[28]'
M = M_Kenya_ho
v = normalize!((M^10)*ones(17),1)



evects[:,end]/sum(evects[:,end])

posterior_ϵ = [res.ϵ for res in results_trans]
scatter(posterior_ϵ,lab = "epsilon")
histogram(posterior_ϵ)
posterior_R = [res.R for res in results_trans]
plot(posterior_R,lab = "R")
histogram(posterior_R,xticks = 0.:0.5:5)
