...
KenyaTbl = readtable("2009_National_Estimates_of_Rural_and_Urban_Populations_by_County.csv")
...
for i = 1:n
    ...
    x = join(split(x,")"))
    y = split(x,",")
    z = parse(y[2])*111.32,parse(y[1])*110.57
    KenyaTbl[:Loc][i] = z
...
# KenyaTbl[:UrbanIndex][1] = KenyaTbl[:Urban][1]/(KenyaTbl[:Urban][1]+KenyaTbl[:Rural][1])
